...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_RS')
	 Begin 
	  Alter Database [Assignment07DB_RS] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_RS;
	 End
	Create Database Assignment07DB_RS;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_RS;
...
SELECT
  P.ProductName
 ,DateName(mm,I.InventoryDate) + ' ' + DateName(year,I.InventoryDate) as 'InventoryDate'
 ,I.Count
...
SELECT TOP 1000000
 ...
 ,P.InventoryDate
 ,P.InventoryCount
 ,[PreviousMonthCount] = IIF(P.InventoryDate Like ('January%'), 0, IsNull(lag(P.InventoryCount) Over (order By P.ProductName,Year(P.InventoryDate)),0) )
...
SELECT Top 1000000
 ...
 ,[CountVsPreviousCountKPI] = IsNull(Case
    ...
    End, 0)
 From vProductInventoriesWithPreviousMonthCounts
 Order by ProductName, Year(InventoryDate), InventoryCount;
 GO
